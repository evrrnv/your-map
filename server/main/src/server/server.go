...
func Run() (err error) {
	...
	r.GET("/ws", wshandler)
	if UseMQTT {
		r.GET("/api/v1/mqtt/:family", handlerMQTT)
	}
	r.POST("/api/v1/gps", handlerGPS)
	r.POST("/data", handlerData)
	r.POST("/classify", handlerDataClassify)
	r.POST("/learn", handlerFIND)
	r.POST("/track", handlerFIND)
